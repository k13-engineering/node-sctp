...
class Association extends EventEmitter {
  ...
  SEND (buffer, options, callback) {
    /*
     ...
     -> result
     */
    this.debugger.debug('API SEND %d bytes, %o', buffer.length, options)
    ...
    this.send(buffer, options, error => {
      ...
      if (error) {
        ...
        if (drain) {
          this.debugger.debug('drain is %s (flightsize %d cwnd %d peer_rwnd %d)', drain, this.flightsize, this.cwnd, this.peer_rwnd)
          callback()
        } else {
  ...
  drain () {
    ...
    const drain = (this.flightsize < this.cwnd) && (this.lastChunkSize + this.flightsize < this.peer_rwnd)
    ...
    */
    ...
    if (buffer.length > mtu) {
      ...
      while (offset < buffer.length) {
        const E = buffer.length - offset <= mtu
        chunk = {
          flags: {
            E: E,
            B: offset === 0,
        ...
        offset += mtu
        // make callback only for the last chunk
        this._sendChunk('data', chunk, null, E ? callback : null)
      }
    } else {
      chunk = {
        flags: {
  ...
  pause (stream) {
    this.reassembly.pause(stream)
  }

  unpause (stream) {
    this.reassembly.unpause(stream)
  }
}
