...
class SCTPStreamWritable extends Writable {
  constructor (socket, streamId, ppid) {
    ...
    this.bytesSentWithoutCallback = 0
  }
  ...
  _write (chunk, encoding, callback) {
    ...
    this.bytesSentWithoutCallback += chunk.length
    let callbackCalled = false

    this.socket.sendToAssociation(chunk, options, () => {
      this.bytesSentWithoutCallback -= chunk.length;

      if (!this.destroyed && !callbackCalled) {
        callbackCalled = true;
        callback()
      }
    })

    if(this.bytesSentWithoutCallback < 1 * 1024 * 1024 && !callbackCalled) {
      callbackCalled = true
      callback()
    }
  }
...
class Socket extends Duplex {
  constructor (options) {
    super({
      ...options,
      writableHighWaterMark: 1 * 1024 * 1024,
      readableHighWaterMark: 1 * 1024 * 1024
    })
    options = options || {}
